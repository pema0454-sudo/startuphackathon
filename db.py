...
def _sb_select(table: str, filters: Optional[dict] = None, order: Optional[str] = None,
                desc: bool = False, limit: Optional[int] = None) -> Optional[list]:
    client = _sb()
    if client is None:
        return None
    try:
...
def get_latest_reading(station: str, parameter: str = "river_level") -> Optional[dict]:
    """Like select(), but asks Postgres to sort+limit itself instead of
    pulling a station/parameter's ENTIRE reading history across the network
    just to keep the last row. This is called ~2x per policy on every
    single agent cycle AND on every dashboard poll (every 6s) — with 35+
    policies, an unbounded fetch here means dozens of full-table transfers
    every few seconds, and the readings table only ever grows (auto-drift
    adds a row every cycle, forever). That was the real cause of Supabase
    responses taking tens of seconds under load, not just network latency."""
    filters = {"station": station, "parameter": parameter}
    with _lock:
        if SUPABASE_CONFIGURED:
            rows = _sb_select("readings", filters, order="timestamp", desc=True, limit=1)
            if rows is not None:
                return rows[0] if rows else None
            # Supabase unreachable this call — fall through to the local cache below
        rows = _local_select("readings", filters)
    if not rows:
        return None
