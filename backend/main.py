...
from stations import DISTRICT, STATIONS

BACKEND_DIR = os.path.dirname(os.path.abspath(__file__))
FRONTEND_DIR = os.path.join(BACKEND_DIR, "..", "frontend")

AGENT_INTERVAL_SECONDS = int(os.getenv("AGENT_INTERVAL_SECONDS", "180"))
...
async def lifespan(app: FastAPI):
    ...
    scheduler.start()
    db.log_trace("startup", "BOOT", f"scheduler started · every {AGENT_INTERVAL_SECONDS}s · "
                                     f"mode={'live-AI' if llm.HACKATHON_KEY else 'DEMO AI MODE (no key)'}")
    yield
    scheduler.shutdown(wait=False)


app = FastAPI(title="Bema Farm", lifespan=lifespan)
