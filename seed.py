"""
seed.py — populate Bima Farm with synthetic, clearly-labelled sandbox data
...
# Payout used to be a flat NPR 5,000/4,200 per hazard type for every single
# policy, which looked fake side by side on the claims desk (every flood
# claim recommending the exact same amount). Real parametric crop cover
# sizes the payout to how much land is actually insured, so instead each
# policy gets a realistic small-holding size in kattha — the land unit
# Dhanusa/Terai farmers actually use (1 kattha ~ 338.63 m^2; 20 kattha = 1
# bigha) — and the payout/premium are derived from that, so two farmers on
# the same station with the same trigger can still recommend different
# amounts, same as a real per-farmer sum-insured would.
LAND_RATE_PER_KATTHA = {"flood": 250, "drought": 200}  # NPR payout per kattha insured
PREMIUM_RATE = 0.10  # flat 10% of payout, matches the original 500/5000 and 420/4200 ratios


def _land_kattha(name: str, i: int) -> float:
    if name == "Sita Tamang":
        return 20.0  # exactly 1 bigha — keeps her pitch figures (spec §1: payout NPR 5,000) exact
    rng = random.Random(2000 + i)  # deterministic per farmer, not re-rolled on every reseed
    return round(rng.uniform(3, 24) * 2) / 2  # 3-24 kattha, nearest half-kattha
...
def build_policies() -> list[dict]:
    ...
    for i, (name, ward, municipality, station, hazard) in enumerate(FARMERS):
        ...
        land = _land_kattha(name, i)
        payout = int(max(500, round(LAND_RATE_PER_KATTHA[hazard] * land, -2)))  # nearest NPR 100, NPR 500 floor
        premium = int(round(payout * PREMIUM_RATE, -1))  # nearest NPR 10
